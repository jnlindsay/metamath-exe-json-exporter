...
// 0.136 10-Oct-2016 mminou.c - fix resource leak bug reported by David Binderman.
...
// 0.127 10-May-2016 metamath.c, mmcmdl.c, mmhlpb.c - added /OVERRIDE to PROVE.
...
// 0.07.69 9-Jul-2011 nm mmpars.c, mmvstr.c - Untab file in WRITE SOURCE ... /REWRAP.
...
// 0.07.53 24-Sep-2010 nm mmveri.c - fixed bug(2106), reported by Michal Burger.
...
// 0.07.47 2-Aug-2009 nm mmwtex.c, mmwtex.h - added user name to mathbox pages.
...
#define SYNTAX 4
  ...
  // Set when the first command line processed; used to exit shell
  // command line mode.
...
"Entering the Text Tools utilities.  Type HELP for help, EXIT to exit.\n");
    ...
    if (cmdMatches("READ")) {
      ...
      // Couldn't open it (error msg was provided by fSafeOpen)
...
#define MIN_SPACE 2 // At least this many spaces between columns
    ...
    if (cmdMatches("SHOW STATEMENT") && (
      ...
      if (((char *)(g_fullArg[2]))[0] == '*' || g_briefHtmlFlag) {
        // -2 is for ASCII table; -1 is for theorems; 0 is for definitions
        s = -2;
      } else {
        s = 1;
      }
      ...
      for (s = s + 0; s <= g_statements; s++) {
        ...
        if (!g_texFilePtr) goto htmlDone; // Couldn't open it (err msg was provided)
        g_texFileOpenFlag = 1;
          ...
          for (i = 1; i <= g_statements; i++) {
            ...
            switch (s) {
              case -2:
                ...
                // It's a $c or $v statement, so each token generates a table row.
...
">>> ?Warning: You are overwriting a proof whose modification is discouraged.\n");
     ...
     "?You have not opened a %s file.  Use the OPEN %s command first.\n",
      ...
      if (i) {
        ...
        // To use as flag; error message will occur in showDetailStep().
...
"---------Clip out the proof below this line to put it in the source file:\n");
            ...
            if (str2[0] == 0) { // The is no contributor, so add one
              ...
              let(&str4, cat("\n", space(indentation + 1),
                  "(Contributed by ", g_contributorName,
                      ", ", str3, ".) ", NULL));
              // If there is a 2nd date below proof, add a "(Revised by... )" tag
              if (str5[0] != 0) {
          ...
          if (saveFlag) {
            ...
            if (processUndoStack(NULL, PUS_GET_STATUS, "", 0)) {
              ...
              // UNDO stack empty no longer reliably indicates that
              // proof hasn't changed.
              proofSavedFlag = 1; 
            }
...
">>> ?Warning: Modification of this statement's proof is discouraged.\n"
    ...
    if (cmdMatches("UNIFY")) {
      ...
      if (cmdMatches("UNIFY STEP")) {
        ...
        // 2nd arg. means print msg if already unified
  ...
  "Steps were unified.  SHOW NEW_PROOF / NOT_UNIFIED to see any remaining.\n");
        ...
        while (1) {
          ...
          autoUnify(0);
          for (s = m - 1; s >= 0; s--) {
            // 2nd arg. means no msg if already unified.
...
">>> ?Warning: You are assigning a statement whose usage is discouraged.\n");
      ...
      // ???We might want to add a setting to defeat this if user doesn't like it.
...
">>> ?Warning: You are assigning a statement whose usage is discouraged.\n");
      ...
      // If proof wasn't complete before (p = 1), but is now, print congrats for user
  ...
  "?Step %ld target has shared dummy variables and cannot be improved.\n", s);
          continue; // Don't try to improve dummy variables that aren't isolated.
        }
        ...
        // If q=1, subproof must be an unknown step, so don't bother to delete it
...
"Pass 2:  Trying to match all statements, with cut-free hypothesis matches...\n"
          ...
          // improveAllIter = 3: run proveByReplacement on steps with incomplete subproofs.
          ...
          for (s = m; s > 0; s--) {
            ...
            // 0 = no dummy vars, 1 = isolated dummy vars, 2 = not isolated
            dummyVarIsoFlag = checkDummyVarIsolation(s - 1);
            // Don't try to improve dummy variables that aren't isolated.
    ...
    if (cmdMatches("MINIMIZE_WITH")) {
      ...
      // Status flag to help determine messages:
      ...
      for (forwRevPass = 1; forwRevPass <= 2; forwRevPass++) {
        ...
        for (k = forwFlag ? 1 : (g_proveStatement - 1);
          ...
          if (forbidMatchList[0]) { // User provided a /FORBID list.
            ...
            // in the forbidMatchList directly (traceProof() won't find this).
          ...
          if (!nmbrEq(nmbrTmp, g_ProofInProgress.proof)) {
            ...
            // if (noNewAxiomsMatchList[0]) { // User provided /NO_NEW_AXIOMS_FROM
              // If we haven't called trace yet for the theorem being proved, do it now.
 ...
 "    but compressed size:  old = %ld bytes, new = %ld bytes\n",
    ...
    "    (Uncompressed steps decreased from %ld to %ld).\n",
      ...
      "Proof of \"%s\" increased from %ld to %ld steps using \"%s\".\n",
            ...
            // Save the changed proof in case we have to restore it later.
            copyProofStruct(&saveProofForReverting, g_ProofInProgress);
          }
        } // Next k (statement)
...
">>> it back ON if you are not intimately familiar with this database.\n");
    ...
    if (cmdMatches("OPEN TEX")) {
      if (g_texDefsRead) {
        if (g_htmlFlag) {
          // Actually it isn't clear to me this is still the case, but to be 
          // safe I left it in.
